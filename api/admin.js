...
<style>:root{--ink:#0a0a0a;--paper:#fff;--warm:#f7f6f2}*{box-sizing:border-box}body{margin:0;background:var(--warm);color:var(--ink);font-family:Arial,Helvetica,sans-serif}.page{width:min(100% - 2rem,72rem);margin:auto;padding:4rem 0}p,code,button{font:700 .76rem/1.4 SFMono-Regular,Consolas,monospace;letter-spacing:.08em;text-transform:uppercase}h1{max-width:10ch;margin:.5rem 0 2rem;font-size:clamp(4rem,10vw,9rem);line-height:.78;letter-spacing:-.1em}h2{margin:.5rem 0 1rem;font-size:clamp(2.5rem,5vw,5rem);line-height:.85;letter-spacing:-.08em}section{border-top:1px solid var(--ink);padding:1.25rem 0 3rem}.cards{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:2rem}.card{border-top:1px solid var(--ink);padding:1rem 0}.tabs{display:flex;flex-wrap:wrap;gap:.5rem;margin:1.25rem 0}button{min-height:3.25rem;padding:0 1rem;border:1px solid var(--ink);background:var(--paper);color:var(--ink);cursor:pointer}button[aria-pressed=true],#copy{background:var(--ink);color:var(--paper)}pre{padding:1rem;border:1px solid var(--ink);background:var(--paper);overflow:auto}.detail{max-width:34rem;font:400 1.05rem/1.35 Arial,Helvetica,sans-serif;letter-spacing:0;text-transform:none}@media(max-width:42rem){.cards{grid-template-columns:1fr}}</style></head>
