...
module.exports = async (request, response) => {
  ...
  if (!passcode || !sessionSecret) {
    response.status(503).end('Admin access is not configured.');
    return;
  }
  ...
  response.writeHead(303, {
    Location: '/admin',
    'Set-Cookie': `opndrm_admin=${sessionValue(sessionSecret)}; Path=/; Max-Age=28800; HttpOnly; Secure; SameSite=Strict`,
  });
  response.end();
};
