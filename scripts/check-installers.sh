...
require "$MAC" 'NO MISTAKES GATE — RESERVED (INACTIVE)'
...
forbid "$MAC" 'omlx'
forbid "$MAC" 'apiKey'
forbid "$MAC" 'no-mistakes run'
forbid "$ROOT/site/index.html" '/admin'
forbid "$ROOT/site/es/index.html" '/admin'
forbid "$ROOT/vercel.json" 'rewrites'
for path in "$ROOT"/README.md "$ROOT"/site/index.html "$ROOT"/site/es/index.html "$ROOT"/skills/opndrm-prime/SKILL.md; do
  forbid "$path" 'QM'
  forbid "$path" 'oMLX'
  forbid "$path" 'Qwen'
done
