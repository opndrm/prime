...
Cuando la instalación tiene éxito, WezTerm se abre conectado a su propia sesión HERDR con nombre para el espacio seleccionado. El instalador crea esa sesión personal únicamente después de que exista la raíz seleccionada y se detiene sin mostrar `Ready` si HERDR no puede iniciar o conectarse. Su espacio de trabajo tiene dos áreas simples:

- **PRIME** — empiece aquí. Dígale a Prime Agent lo que quiere crear.
- **NO MISTAKES GATE — RESERVED (INACTIVE)** — permanece inactivo hasta que usted decida usarlo para verificar una rama terminada.
