...
<section><p>Choose the app assigned to you. Copy its Mac installer, then follow that app’s work card.</p><div class="tabs"><button data-lane="ADAM" aria-pressed="true">ADAM</button><button data-lane="FRNKLY.ONE" aria-pressed="false">FRNKLY.ONE</button></div><pre><code id="command"></code></pre><button id="copy">COPY MAC INSTALL</button></section>
...
<script>let lane='ADAM';const render=()=>{document.querySelectorAll('[data-lane]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.lane===lane)));document.querySelector('#command').textContent='sudo -v && curl -fsSL https://opndrm.com/install-macos.sh | bash -s -- '+lane;};document.querySelectorAll('[data-lane]').forEach(b=>b.onclick=()=>{lane=b.dataset.lane;render()});document.querySelector('#copy').onclick=async()=>{await navigator.clipboard.writeText(document.querySelector('#command').textContent);document.querySelector('#copy').textContent='COPIED — PASTE IN TERMINAL'};render();</script></body></html>`);
};
