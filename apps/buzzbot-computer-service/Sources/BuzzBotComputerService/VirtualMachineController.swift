        ...
        machine.start { [weak self, machine] result in
            Task { @MainActor [weak self, machine] in
                ...
                case .success:
                    ...
                    FileHandle.standardError.write(Data("BuzzBot: VM started successfully\n".utf8))
    ...
    private func setupGuestSocketClient(_ machine: VZVirtualMachine) {
        ...
        for device in machine.socketDevices {
            if let virtioDevice = device as? VZVirtioSocketDevice {
                ...
                FileHandle.standardError.write(Data("BuzzBot: guest socket client ready\n".utf8))
                return
            }
        }
        FileHandle.standardError.write(Data("BuzzBot: no VZVirtioSocketDevice found\n".utf8))
    }
